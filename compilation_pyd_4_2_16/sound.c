...
)
{
   ...
   switch( dev->driver )
   {
      case DEV_DRIVER_PORTAUDIO:
         ...
         break;
      case DEV_DRIVER_ALSA:
         quisk_play_alsa(dev, nSamples, cSamples, report_latency, volume);//-------------------------------------------------------- использовать ALSA в Windows
         break;
      case DEV_DRIVER_PULSEAUDIO:
...
static int open_radio_sound_socket(void)
{
	...
	if (radio_sound_ip[0] == 0 && radio_sound_mic_ip[0] == 0)
	    //win_sock_error = 1; // ------------------------------------------------------------------------------------------------- RA3PKJ
		return win_sock_error;
#ifdef MS_WINDOWS
	wVersionRequested = MAKEWORD(2, 2);
	if (WSAStartup(wVersionRequested, &wsaData) != 0) {
		printf("open_radio_sound_socket: Failure to start WinSock\n");
		//win_sock_error = 1; // ------------------------------------------------------------------------------------------------- RA3PKJ
		return win_sock_error;		// failure to start winsock
	}
#endif
	if (radio_sound_ip[0]) {
	...
	return win_sock_error ; // -------------------------------------------------------------------------------------------- RA3PKJ
}

int quisk_open_sound(void)	// Called from GUI thread
{
	int i;
	...
	win_sock_error = open_radio_sound_socket(); // --------------------------------------------------------------------------------------------- RA3PKJ
#if DEBUG_IO
