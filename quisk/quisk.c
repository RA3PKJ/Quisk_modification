...
int ssbTxLow; // ----------------------------------- добавлено --------------------- кнопка SSBtx Low ------- 45 RA3PKJ
...
static PyObject * set_ssb_low(PyObject * self, PyObject * args)
{
	int TxLow;
	if (!PyArg_ParseTuple (args, "i", &TxLow))
		return NULL;
    ssbTxLow = TxLow; //ssbTxLow глобальная переменная   
	tx_filter(NULL, 0);
	return Py_None;
}
...
static PyMethodDef QuiskMethods[] = {
    {"set_ssb_low", set_ssb_low, METH_VARARGS, "Set SSB tx low."}, // ----------------- добавлено --------------- кнопка SSBtx Low ------- 45 RA3PKJ
