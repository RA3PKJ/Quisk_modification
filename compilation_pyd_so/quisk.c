...
{	// Changes here will require changes to get_filter_rate();
	...
	double d, di, dd;
	//static struct AgcState Agc1 = {0.3, 16000, 0}, Agc2 = {0.3, 16000, 0}; // ------------------------------------- удалено для freedv --------------------------- RA3PKJ
	static struct stStorage {
	...
	case 9:     // digital mode DGT-IQ at 48 ksps
		...
		break;
//	case 11:	 // digital voice at 8 ksps //------------------------------------------------------------- удалено для freedv ------------------------------------- RA3PKJ
//	case 12:
...
static PyObject * open_sound(PyObject * self, PyObject * args)
{
	...
	int win_sock_error = 0; // --------------------------------------------------------------------------------- добавлена переменная ------------------------------ RA3PKJ
	...
	//quisk_open_sound();
	win_sock_error = quisk_open_sound(); // --------------------------------------------------------- теперь вызов функции с получением значения переменной -------- RA3PKJ
	quisk_open_mic();
	//return get_state(NULL, NULL);
	return Py_BuildValue("i", win_sock_error); // --------------------------------------------------------- вернуть значение переменной ---------------------------- RA3PKJ
}
...
static PyObject * get_graph(PyObject * self, PyObject * args)	// Called by the GUI thread
{
	...
	if (!PyArg_ParseTuple (args, "idd", &k, &zoom, &deltaf))//проверка правильности переданных в фунуцию аргументов (четыре аргумента)
		return NULL;
	...
	for (ffts = 0; ffts < FFT_ARRAY_SIZE; ffts++) {
		...
		if (scan_blocks)
          if (split_rxtx)//если Split выключен, то S-метр показывает в полосе передатчика
		  ...
		  if (split_rxtx)//если Split выключен, то S-метр показывает в полосе передатчика
			...
			d1 = (double)rx_tune_freq * fft_size / fft_sample_rate;
...
static PyObject * get_filter(PyObject * self, PyObject * args)
{
	...
	tuple2 = PyTuple_New(data_width); //convert to PyObject
	i = 0;
...
//	free(bufQ);     //free up memory (C function)
//	free(bufI);     //free up memory (C function)
//	free(average);  //free up memory (C function)
...
//	{"freedv_open", quisk_freedv_open, METH_VARARGS, "Open FreeDV."}, //- ---------------------------------- удалено для freedv ------------------------------------ RA3PKJ
